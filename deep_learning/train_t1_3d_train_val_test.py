...
# ---------- 1. Dataset / transforms (T1 only) ----------

class MotumT1Dataset(Dataset):
    """
    One row in df has: subject, path, label.
    We load T1 from `path` -> [1, H, W, D].
    """
    ...
    def __getitem__(self, idx):
        ...
        data = {
            "image": row["path"],     # <--- use 'path' column
            "label": row["label"],
        }
        if self.transforms is not None:
            data = self.transforms(data)
        return {
            "image": data["image"],   # [1, 128,128,128]
            "label": data["label"],
            "subject": row["subject"],
        }
...
def get_transforms(train=True):
    """
    Load T1 from 'image', normalize, crop foreground,
    resize to 128^3. Single-channel volume.
    """
    base = [
...
def get_model(in_channels=1):
    model = resnet18(
        spatial_dims=3,
        n_input_channels=in_channels,  # 1 = T1 only
...
def train_one_fold(df, train_idx, val_idx, fold, epochs=60, batch_size=2, lr=1e-4):
    ...
    model = get_model(in_channels=1).to(DEVICE)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in train_loader:
            images = batch["image"].to(DEVICE)       # [B,1,128,128,128]
        ...
        if val_auc > best_val_auc:
            ...
            torch.save(best_state_dict, f"best_resnet_t1_fold{fold}.pt")
...
def train_full_and_eval_test(df_train_val, df_test, epochs=60, batch_size=2, lr=1e-4):
    print("\n===== Training final T1-only model on full train+val, evaluating on test =====")

    train_ds = MotumT1Dataset(df_train_val, transforms=get_transforms(train=True))
    test_ds  = MotumT1Dataset(df_test,      transforms=get_transforms(train=False))
    ...
    print("\n===== Test set performance (final T1-only model) =====")
    ...
    test_df.to_csv("test_predictions_3dresnet_t1.csv", index=False)
    print("\nSaved test predictions to test_predictions_3dresnet_t1.csv")

    np.save("test_labels_3dresnet_t1.npy", test_labels)
    np.save("test_probs_3dresnet_t1.npy",  test_probs)
    np.save("test_preds_3dresnet_t1.npy",  test_preds)
    torch.save(model.state_dict(), "final_resnet_t1_trainval.pt")
...
def main():
    ...
    print("\n=== 5-fold CV on train+val (T1 only) ===")
    ...
    np.save("cv_labels_3dresnet_t1.npy", all_labels)
    np.save("cv_probs_3dresnet_t1.npy",  all_probs)
    np.save("cv_preds_3dresnet_t1.npy",  all_preds)
